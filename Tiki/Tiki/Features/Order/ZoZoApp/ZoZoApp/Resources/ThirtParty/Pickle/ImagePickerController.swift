        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            DispatchQueue.main.async {
                picker.dismiss(animated: true) {
                    self.imagePickerDelegate?.imagePickerCpmtroller?(self,
                                                                     didTakePhoto: croppedImage ?? originalImage)
                }
            }
        }
...
fileprivate extension ImagePickerController {
    ...
    fileprivate func handle(photoLibraryPermission status: PHAuthorizationStatus) {
        ...
        case .denied, .restricted:
            AlertManager.shared.show(TextManager.alertTitle.localized(), message: TextManager.photoPermission, buttons: [TextManager.IUnderstand, TextManager.cancel]) { (action, index) in
                if index == 0 {
    ...
    fileprivate func launchCamera() {
        ...
        case .denied, .restricted:
            AlertManager.shared.show(TextManager.alertTitle.localized(), message: TextManager.cameraPermission, buttons: [TextManager.IUnderstand, TextManager.cancel]) { (action, index) in
                if index == 0 {
                    UIApplication.shared.open(URL(string: UIApplication.openSettingsURLString)!)
                }
            }
