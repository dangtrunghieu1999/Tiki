 ...
 */
open class AbstractHeaderedTabScrollViewController: BaseViewController {
    ...
    public var headerHeight: CGFloat = UserProfileHeaderView.estimatedHeight {
        didSet {
    ...
    /// Param indicate hidden Navi or not
    public var hiddenNavigationBar = false
    ...
    override open func viewDidLoad() {
        ...
        if hiddenNavigationBar {
            ...
            if #available(iOS 11, *) {
                headerTopConstraint = headerContainer.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor)
            } else {
                headerTopConstraint = headerContainer.topAnchor.constraint(equalTo: topLayoutGuide.bottomAnchor)
            }
        }
    ...
    override open func viewWillDisappear(_ animated: Bool) {
        ...
        if let navCtrl = self.navigationController, hiddenNavigationBar {
    ...
    public func pleaseScroll(_ scrollView: UIScrollView) {
        ...
        let minY: CGFloat = headerHeight + navBarOffset()
    ...
    func navBarOffset() -> CGFloat {
        return (self.navigationController?.navigationBar.bounds.height ?? 40) + UIApplication.shared.statusBarFrame.height
    }
