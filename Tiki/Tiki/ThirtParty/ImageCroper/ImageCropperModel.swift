  ...
  var parentFrame: CGRect {
    ...
    set {
      ...
      let figureSize = configuration.figure.maskSize(with: newValue.size,
                                                     ratio: configuration.customRatio)
      figureFrame = CGRect(x: (newValue.width - figureSize.width) / 2,
                           y: (newValue.height - figureSize.height) / 2,
...
extension ImageCropperConfiguration.ImageCropperFigureType {
  fileprivate func maskSize(with parentSize:CGSize, ratio: CGSize?) -> CGSize {
    ...
    switch self {
    case .circle, .square:
      width = parentVertical ? parentSize.width * 0.9 : parentSize.height * 0.9
      height = width
    case .rect2x1:
